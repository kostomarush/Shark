...
def scan():
    ...
    if response.mode == 'SYN':
        ...
        for ports in open_ports:
            ...
            stub.scan(prot_pb2.DataClient(id_client='1',ip_status=ip_status, protocols=protocols,open_ports=f'{ports}', state = nm[ip_address]['tcp'][ports]['state']))
    ...
    if response.mode == 'UDP':
        ...
        for ports in open_ports:
            stub.scan(prot_pb2.DataClient(id_client='1', ip_status=ip_status, protocols=protocols,open_ports=f'{ports}',state = nm[ip_address]['udp'][ports]['state']))
